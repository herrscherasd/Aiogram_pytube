...
from keyboards import share_button
...
async def share(message:types.Message):
    await message.answer(f"{message.from_user.full_name} вы можете поделиться контактом", reply_markup=share_button)
...
async def get_contact(message:types.Message):
    ...
    connect.commit()
    await message.answer('Ваш контакт записан.')

# @dp.message_handler(content_types=types.ContentType.LOCATION)
# async def get_location(message:types.Message):
#     bot.send_message(731982105, 'Лалалала')
